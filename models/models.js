...
$("#brightnessRange").mousemove(
    function () {
        //console.log($("#brightnessRange")[0].value);
        brightnessDiff = $("#brightnessRange")[0].value / 100.0;
        if (savedData == null)
            changeBlueValue();
        else
    ...
    for (let i = 0; i < canvasOrig.height; ++i) {
        for (let j = 0; j < canvasOrig.width; ++j) {
            ...
            if (mask[i][j] === 1) {
                ...
                //if this pixel is blue  
                if (checkHueTolerance(blueHueNorm, hueTolerance, pixelHsv.h, pixelHsv.s)) {
...
function checkHueTolerance(neededHue, tolerance, currentHue, currentSat) {
    //180-280 = blue
    if (currentHue * 360 >= 180 && currentHue * 360 <= 240 && currentSat>=0.01) return true;
    return false;
}
